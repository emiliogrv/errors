...
func (receiver *StructuredError) asString(stringsBuilder *strings.Builder, depth int) {
	...
	if len(receiver.Tags) > zero {
		...
		sliceToString(stringsBuilder, depth, tagsKey, receiver.Tags)
	}
	...
	if len(receiver.Stack) > zero {
		...
		stringsBuilder.WriteString(stackKey)
		stringsBuilder.WriteString(equals)

		stackStr := string(receiver.Stack)
		stackLines := strings.Split(stackStr, newLine)
		_depth := depth + one

		for index, line := range stackLines {
			if line == "" {
				continue
			}
			...
			if index > zero {
				stringsBuilder.WriteString(newLine)
				tabToString(stringsBuilder, _depth)
			}
